...
function createComment(url,request){
  const newRequest = request.body && request.body.comment
  const response = {};
  if(newRequest && database.users[newRequest.username] && newRequest.body && newRequest.articleId && database.articles[newRequest.articleId]){
    const newComment = {
      id: database.nextCommentId,
      body: newRequest.body,
      username: newRequest.username,
      articleId: newRequest.articleId,
      upvotedBy: [],
      downvotedBy: []
    }
...
function deleteComment (url,request){
  ...
  if(savedComment){
    ...
    for(let artId in database.articles){
      database.articles[artId].commentIds.splice(database.articles[artId].commentIds.indexOf(id),1)
    }
    
    for(let uId in database.users){
      database.users[uId].commentIds.splice(database.users[uId].commentIds.indexOf(id),1)
    }
